...
/*
          	...
          	for (unsigned int i=0; i<phi_face_f.size(); i++){
              	for (unsigned int j=0; j<phi_face_p.size(); j++){
					Kxp(i,j) += JxW_face_f[qp]*phi_face_f[i][qp]*face_normals_f[qp](0)*phi_face_p[j][qp];
					Kyp(i,j) += JxW_face_f[qp]*phi_face_f[i][qp]*face_normals_f[qp](1)*phi_face_p[j][qp];
					#if THREED
					Kzp(i,j) += JxW_face_f[qp]*phi_face_f[i][qp]*face_normals_f[qp](2)*phi_face_p[j][qp];
					#endif
		      	}
		    }
...
*/
			...
			for (unsigned int i=0; i<phi_face_p.size(); i++){				
				Fx(i) +=  0*penalty/h_elem*JxW_face_f[qp]*face_normals_f[qp](0)*phi_face_f[i][qp]*face_normals_f[qp](0);
				Fy(i) +=  0*penalty/h_elem*JxW_face_f[qp]*face_normals_f[qp](1)*phi_face_f[i][qp]*face_normals_f[qp](1);
				#if THREED
          		Fz(i) += 0*penalty/h_elem*JxW_face_f[qp]*face_normals_f[qp](2)*phi_face_f[i][qp]*face_normals_f[qp](2);
				#endif
			}
