...
//std::cout<< t <<std::endl;
//std::cout<< "val " << ((-1.0/(4.0*PIE))*cos(2.0*PIE*x)*sin(2.0*PIE*y)*sin(2.0*PIE*t))/FAC <<std::endl;
...
#endif
#if SIN && !THREED && !TIME
  return (-2*PIE*cos(2*PIE*x)*sin(2*PIE*y))/FAC;
#endif

#if SIN && !THREED && TIME
  return (-2*PIE*cos(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC;
#endif

#if THREED
  return (-2*PIE*cos(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*z))/FAC;
#endif
}
...
#if SIN && !THREED && !TIME
  return (-2*PIE*sin(2*PIE*x)*cos(2*PIE*y))/FAC;
#endif

#if (SIN && !THREED) && TIME
  return (-2*PIE*sin(2*PIE*x)*cos(2*PIE*y)*sin(2*PIE*t))/FAC;

  
#endif

#if THREED
  return (-2*PIE*sin(2*PIE*x)*cos(2*PIE*y)*sin(2*PIE*z))/FAC;
#endif
}
                         ...
                         const std::string&) // unk_name, not needed
{
  ...
  // analytic solution value
#if THREED
  return (-2*PIE*sin(2*PIE*x)*sin(2*PIE*y)*cos(2*PIE*z))/FAC;
#endif
}
...
#if SIN && !THREED && !TIME
  gradx(0) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y))/FAC;
  gradx(1) = (-4*PIE*PIE*cos(2*PIE*x)*cos(2*PIE*y))/FAC;
  return gradx;
#endif

#if SIN && !THREED && TIME
  gradx(0) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC;
  gradx(1) = (-4*PIE*PIE*cos(2*PIE*x)*cos(2*PIE*y)*sin(2*PIE*t))/FAC;
  return gradx;
#endif

#if THREED
  gradx(0) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*z))/FAC;
  gradx(1) = (-4*PIE*PIE*cos(2*PIE*x)*cos(2*PIE*y)*sin(2*PIE*z))/FAC;
  gradx(2) = (-4*PIE*PIE*cos(2*PIE*x)*sin(2*PIE*y)*cos(2*PIE*z))/FAC;
  return gradx;
#endif
}
...
#if SIN && !THREED && !TIME
  grady(0) = (-4*PIE*PIE*cos(2*PIE*x)*cos(2*PIE*y))/FAC;
  grady(1) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y))/FAC;
  return grady;
#endif

#if SIN && !THREED && TIME
  grady(0) = (-4*PIE*PIE*cos(2*PIE*x)*cos(2*PIE*y)*sin(2*PIE*t))/FAC;
  grady(1) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC;
  return grady;
#endif

#if THREED
  grady(0) = (-4*PIE*PIE*cos(2*PIE*x)*cos(2*PIE*y)*sin(2*PIE*z))/FAC;
  grady(1) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*z))/FAC;
  grady(2) = (-4*PIE*PIE*sin(2*PIE*x)*cos(2*PIE*y)*cos(2*PIE*z))/FAC;
  return grady;
#endif
}
...
#if THREED
  gradz(0) = (-4*PIE*PIE*cos(2*PIE*x)*sin(2*PIE*y)*cos(2*PIE*z))/FAC;
  gradz(1) = (-4*PIE*PIE*sin(2*PIE*x)*cos(2*PIE*y)*cos(2*PIE*z))/FAC;
  gradz(2) = (4*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*z))/FAC;
  return gradz;
#endif
}
...
t=FIX_T;
  ...
  // return ((1/2)*sin(2*PIE*x)*sin(2*PIE*y))/FAC  + ((-1/2)*cos(2*PIE*x)*cos(2*PIE*y))/FAC  + ((4*PIE*PIE)*sin(2*PIE*x)*sin(2*PIE*y))/FAC + ((-4*PIE*PIE)*cos(2*PIE*x)*cos(2*PIE*y))/FAC  ;
  ...
  //Just Darcy
  // return (8*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y))/FAC   ;
...
//return (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC   + (dt*8*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC   - (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*(t-dt)))/FAC ;


  return (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t)*2*PIE*cos(2*PIE*t))/FAC   + (8*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC   ;
   
 //  if(t>dt){

  // return (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC   + dt*(8*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC - (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*(t-dt)))/FAC  ;
  //}else{
    //   return (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC   + dt*(8*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*t))/FAC;
  //}
   ...
   #if THREED
  //BOTH STOKES AND DARCY DIV terms
  return (sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*z))/FAC + (12*PIE*PIE*sin(2*PIE*x)*sin(2*PIE*y)*sin(2*PIE*z))/FAC   ;
  #endif
