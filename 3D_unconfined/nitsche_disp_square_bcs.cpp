for (unsigned int s=0; s<elem->n_sides(); s++)
{
   if (elem->neighbor(s) == NULL)
	...
	if(   (xf<0.001) )  {
		    ...
		    // (u.n,q)_{Gamma} term //Do again for displacement
          	for (unsigned int i=0; i<phi_face_f.size(); i++){
              	for (unsigned int j=0; j<phi_face_p.size(); j++){
					Kup(i,j) += JxW_face_f[qp]*phi_face_f[i][qp]*face_normals_f[qp](0)*phi_face_p[j][qp];
					Kvp(i,j) += JxW_face_f[qp]*phi_face_f[i][qp]*face_normals_f[qp](1)*phi_face_p[j][qp];
					#if THREED
					Kwp(i,j) += JxW_face_f[qp]*phi_face_f[i][qp]*face_normals_f[qp](2)*phi_face_p[j][qp];
					#endif
		      	}
		    }
			...
			disp_value(2)=0;
				...
				for (unsigned int i=0; i<phi_face_f.size(); i++){				
					Fu(i) -=  disp_value(0)*JxW_face_f[qp] * dphi_face_f[i][qp] * (face_normals_f[qp]); 
					Fv(i) -=  disp_value(1)*JxW_face_f[qp] * dphi_face_f[i][qp] * (face_normals_f[qp]); 
					#if THREED
          Fw(i) -=  disp_value(2)*JxW_face_f[qp] * dphi_face_f[i][qp] * (face_normals_f[qp]); 
					#endif
				}

				//(u0,penalty/h_elem*v)_D    
				for (unsigned int i=0; i<phi_face_p.size(); i++){				
					Fu(i) +=  disp_value(0)*JxW_face_f[qp]* penalty/h_elem * phi_face_f[i][qp];
					Fv(i) +=   disp_value(1)*JxW_face_f[qp]* penalty/h_elem * phi_face_f[i][qp];
					#if THREED
          			Fw(i) +=   disp_value(2)*JxW_face_f[qp]* penalty/h_elem * phi_face_f[i][qp];
					#endif
				}
			...
			for (unsigned int i=0; i<phi_face_p.size(); i++){				
				Fp(i) +=  disp_value(0)*JxW_face_f[qp]*face_normals_f[qp](0)*phi_face_p[i][qp];
				Fp(i) +=  disp_value(1)*JxW_face_f[qp]*face_normals_f[qp](1)*phi_face_p[i][qp];
				#if THREED
          		Fp(i) +=  disp_value(2)*JxW_face_f[qp]*face_normals_f[qp](2)*phi_face_p[i][qp];
				#endif
			}
